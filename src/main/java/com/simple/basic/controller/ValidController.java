...
public class ValidController {
	...
	public String validForm(@Valid ValidVo vo, Errors errors, Model model) {
		...
		if(errors.hasErrors()) { //유효성 감사에 실패하면 true
			...
			for ( FieldError err : list ) {
				...
				//에러메시지를 모델에 담는다
				if(err.isBindingFailure()) { // 타입이 맞지 않아서 바인딩에 실패한 경우
					model.addAttribute("valid_" + err.getField(), "형식을 일치시키세요");
				} else {
					model.addAttribute("valid_" + err.getField() , err.getDefaultMessage() );
				}
			}
			model.addAttribute("vo", vo); // 유효성 검사에 실패하더라도 통과한 값은 그대로 모델에 남아있도록 
			
			return "valid/view" ; // 유효성 검사 실패시 다시 원래 화면으로
			
			
		}
		
		return "valid/result"; //유효성 검사를 성공시 나가는 화면
	}
	...
	public String quizForm(@Valid MemberVo vo, Errors errors, Model model) {
		...
		if(errors.hasErrors()) { //유효성 감사에 실패하면 true
			...
			for ( FieldError err : list ) {
				...
				//에러메시지를 모델에 담는다
				if(err.isBindingFailure()) { // 타입이 맞지 않아서 바인딩에 실패한 경우
					model.addAttribute("valid_" + err.getField(), "양식을 일치시키세요");
				} else {
					model.addAttribute("valid_" + err.getField() , err.getDefaultMessage() );
				}
			}
			...
			return "valid/quiz01" ; // 유효성 검사 실패시 다시 원래 화면으로
			
			
		}
		
		return "valid/quiz01_result"; //유효성 검사를 성공시 나가는 화면
